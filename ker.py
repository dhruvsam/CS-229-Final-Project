...
def get_stats(train,label=1):
    ...
    for x in train['band_'+str(label)]:
        image=np.array(x).reshape(75,75)
        mean_mag, mean_ang, std_mag, std_ang, skew_mag, skew_ang, kurt_mag, kurt_ang=statsOfGradients(image)
        hog1.append(mean_mag)
        hog2.append(std_mag)
        hog3.append(skew_mag)
        hog4.append(kurt_mag)
        ...
        i=i+1
    train['hogm_'+str(label)]=hog1
    train['hogstd_'+str(label)]=hog2
    train['hogskew_'+str(label)]=hog3
    len(hog4)
    train['hogkur_'+str(label)]=hog4
